      ...
      <div className='list-comments'>
         ...
         <div>
            {props.comments ? props.comments.map((comment, index) => {
               return (<li key={comment._id}>{comment.text}<div><LikeDislikeReplyComment comments={props.comments} index={index} comment={props.comment} handleReplyCommentClick={props.handleReplyCommentClick} likes={props.likes} dislikes={props.dislikes} handleLike={props.handleLike} handleDislike={props.handleDislike} /></div></li>)
            }) : ''}
         </div>
      </div>
   )
